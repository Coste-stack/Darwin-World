...
import com.myapp.area.Grassfield;
import com.myapp.chart.Plot;
...
public class Board {
    ...
    private int calcAnimalDistToNorthPole(Animal animal) {
        ...
        if (boardMatrix[animal.getPosition().getX()][animal.getPosition().getY()].getArea().getType().equals("com.myapp.area.pole.NorthPole")) {
            return 0;
        }
        // Calculate the distance using one axis
        int i;
        for (i = animal.getPosition().getY(); i >= 0; i--) {
            if (boardMatrix[animal.getPosition().getX()][i].getArea().getType().equals("com.myapp.area.pole.NorthPole")) {
                break;
            }
        }
        return i;
    ...
    private int calcAnimalDistToSouthPole(Animal animal) {
        // Check if current position is at pole
        if (boardMatrix[animal.getPosition().getX()][animal.getPosition().getY()].getArea().getType().equals("com.myapp.area.pole.SouthPole")) {
            return 0;
        }
        // Calculate the distance using one axis
        int i;
        for (i = animal.getPosition().getY(); i < this.BOARD_HEIGHT; i++) {
            if (boardMatrix[animal.getPosition().getX()][i].getArea().getType().equals("com.myapp.area.pole.SouthPole")) {
                break;
            }
        }
        return i;
